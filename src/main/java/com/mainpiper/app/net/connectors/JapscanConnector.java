...
public class JapscanConnector extends HtmlConnector {
		  ...
		  while(it.hasNext()){
			  ...
			  if(href.contains("mangas"))
				  result.put(op.text(), href);
  				  log.debug("AbstractManga {0} with link {1} has been successfully add to the Map", op.text(), op.absUrl("href"));
		  }
	...
	public Map<String, String> getChaptersUrl() {
		...
		//Variable Initialization
		   ...
		   Elements option = ConnectorUtils.tryConnect(connection, this.mangaUrl, "a");
		   if(option == null)
			   return null;
